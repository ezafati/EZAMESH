...
def dt_algo(vmesh):
    ...
    vmesh.point_list = []
    n = 0
    for el in vmesh.triangle_list:
        for k in range(len(el)):
            if el[k] not in set_point_tmp.keys():
                vmesh.point_list.append(plist[el[k]])
...
def boundary_enforc(plist, triangle_list, triangle_list_final, boundary):
    for l in boundary:
        ...
        for n1 in triangle_list_final.keys():
            ...
            if len(points_set) >= 1 and (len(points_set) == 2 or check_intersection(plist, l, set(
                    triangle_list[n1].points).difference(points_set))):
                break
        ...
        if len(points_set) == 1:
            ...
            while l != set(triangle_list[n1].points).intersection(set(triangle_list[n2].points)):
                ...
                if check_intersection(plist, l, set(triangle_list[n1].points).difference(points_set)):
                    n2 = [m for m in triangle_list[n1].adjacent if
                          len(inter_seg.intersection(set(triangle_list[m].points))) > 1][0]
...
def check_in_triangle(p, trian):
    ...
    if (p.x * u1.x + p.y * u1.y <= A.x * u1.x + A.y * u1.y + eps) and (
            p.x * u2.x + p.y * u2.y <= A.x * u2.x + A.y * u2.y + eps) and (
            p.x * u3.x + p.y * u3.y <= B.x * u3.x + B.y * u3.y + eps):
        return 1
    else:
        return 0
